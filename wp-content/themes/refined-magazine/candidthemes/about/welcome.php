<?php
	if(!class_exists('Refined_Magazine_Welcome')) :

		class Refined_Magazine_Welcome {
			...
			public function __construct( $plugins, $strings ) {
				...
				$this->tab_sections = array(
					'getting_started' 		=> esc_html__('Getting Started', 'refined-magazine'),
					'recommended_plugins' 	=> esc_html__('Recommended Plugins', 'refined-magazine'),
					'free_vs_pro'			=> esc_html__('Free Vs Pro','refined-magazine'),
					'support' 				=> esc_html__('Support', 'refined-magazine'),
			...
			<p>
			<?php
			printf( '%1$s %2$s %3$s <a href="%4$s">%5$s</a> %6$s', esc_html__( 'Welcome! Thank you for choosing', 'refined-magazine' ), esc_html($this->theme_name), esc_html__( 'Please make sure you visit our', 'refined-magazine' ), esc_url( admin_url( 'themes.php?page=welcome-page' ) ), esc_html__( 'Welcome Page', 'refined-magazine' ), esc_html__( 'to get started with Refined Magazine.', 'refined-magazine' ) );
			?>
			</p>
			<p><a class="button button-primary" href="<?php echo esc_url(admin_url( 'themes.php?page=welcome-page' )) ?>"><?php esc_html_e( 'Lets Get Started', 'refined-magazine' ); ?></a></p>
			</div>
			<?php
			...
			public function welcome_styles_and_scripts() {
				wp_enqueue_style( 'refined-magazine' . '-welcome-screen', get_template_directory_uri() . '/candidthemes/about/css/about.css' );
				wp_enqueue_script( 'refined-magazine' . '-welcome-screen', get_template_directory_uri() . '/candidthemes/about/js/about.js', array( 'jquery' ) );

				wp_localize_script( 'refined-magazine' . '-welcome-screen', 'CandidWelcomeObject', array(
			...
			public function plugin_installer_callback(){
				...
				if ( ! current_user_can('install_plugins') ) {
					wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'refined-magazine' ) );
				}
				...
				if (! wp_verify_nonce( $nonce, 'plugin_installer_nonce' )) {
					wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'refined-magazine') );
				}
			...
			public function plugin_activation_callback() {
				...
				if(file_exists($plugin_file)) {
					...
					echo esc_html__('Plugin Does not Exists' , 'refined-magazine');
				}
			...
			public function all_required_plugins_installed() {
		  	...
		  	public function get_local_dir_path($plugin) {
	      		...
	      		$url = wp_nonce_url(admin_url('themes.php?page=' . 'refined-magazine' . '-welcome&section=actions_required'),'remote-file-installation');
	      	...
	      	public function admin_sidebar_contents(){ ?>
	      		<div class="candid-admin-side-wrapper">
                    <div class="candid-side-box-wrapper candid-welcome-box-white">
                        <div class="box-header op-box-header"><?php esc_html_e('Video Documentation','refined-magazine'); ?></div>
                        <div class="box-content">
                            <p><?php esc_html_e('Check video documentation to know more about the theme. ','refined-magazine'); ?></p>
                            <a href="<?php echo esc_url($this->strings['video_link']);?>" class="button button-primary" target="_blank"><?php esc_html_e('See Video','refined-magazine'); ?></a>
                        </div>
                    </div>

	      			<div class="candid-side-box-wrapper candid-welcome-box-white">
						<div class="box-header op-box-header"><?php esc_html_e('Buy Pro','refined-magazine'); ?></div>
						<div class="box-content">
							<p><?php esc_html_e('With Refined Magazine Pro make awesome magazine or news site in a minute. ','refined-magazine'); ?></p>
							<a href="<?php echo esc_url($this->strings['pro_link']);?>" class="button button-primary" target="_blank"><?php esc_html_e('Details','refined-magazine'); ?></a>
						</div>
					</div>

					<div class="candid-side-box-wrapper candid-welcome-box-white">
						<div class="box-header op-box-header"><?php esc_html_e('Theme Documentation','refined-magazine'); ?></div>
						<div class="box-content">
							<p><?php esc_html_e('We have online documentation to make your configuration process easy, go through the documentation to get more familiar with theme','refined-magazine'); ?></p>
							<a href="<?php echo esc_url($this->strings['doc_link']);?>" class="button button-primary" target="_blank"><?php esc_html_e('Read Now','refined-magazine'); ?></a>
						</div>
					</div>

					<div class="candid-side-box-wrapper candid-welcome-box-white">
						<div class="box-header op-box-header"><?php printf(esc_html__('Loving %1$s ? ','refined-magazine'),$this->theme_name); ?></div>
						<div class="box-content">
							<p><?php esc_html_e('If you are enjoying theme,spread your love and rate our theme.','refined-magazine'); ?></p>
							<a href="https://wordpress.org/support/theme/refined-magazine/reviews/#new-post" class="button button-primary" target="_blank"><?php esc_html_e('Rate Now','refined-magazine'); ?></a>
						</div>
					</div>
